...
def get_heatmap(
    ...
    for stage in stages:
        ...
        if image is not None and save_path is not None:
            # temp = torch.mean(vis_attn_matrix, dim=0)
            temp = vis_attn_matrix[0]
            a = int(temp.numel() ** 0.5)
            temp = temp.reshape(a, a)
            token_string = f"vis_{stage}"
            # Resize the heatmap to match the original image size
            resized_heatmap = F.interpolate(
                temp.unsqueeze(0).unsqueeze(0),  # Add batch and channel dimensions
