...
from llava.model.language_model.llava_recursion import LlavaRecursionConfig, LlavaLlamaForRecursion
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, merging=None, recursion_config=None, **kwargs):
            ...
            if "mixtral" in model_name.lower():
                ...
                if overwrite_config is not None:
                    ...
                    for k, v in overwrite_config.items():
                        setattr(llava_cfg, k, v)
                #model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                print(recursion_config)
                #model = LlavaLlamaForRecursion.from_pretrained(model_path, recursion_config=recursion_config, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
