...
class LlavaMetaModel:
    ...
    def get_downsampled_vision_tower2(self):
        downsampled_vision_tower2 = getattr(self, "downsampled_vision_tower2", None)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_downsampled_images2(self, images):
        image_features = self.get_model().get_downsampled_vision_tower2()(images)
        # image_features = self.get_model().vision_resampler(image_features, images=images)
        image_features = self.get_model().mm_projector(image_features)
        return image_features
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        if "maxpool2x2" in mm_patch_merge_type:
                            ...
                            if image_mask is not None:
                                ...
                                raise ValueError("recursion_stage2 should not have image_mask")
                                image_feature = image_feature.permute(0, 2, 1, 3, 4).contiguous()
                                image_feature = image_feature.flatten(0, 3)
                        elif generation_type == "downsampled_tot":
                            image_feature = image_feature.permute(0, 2, 1, 3, 4).contiguous()
                        ...
                        if "nobase" in mm_patch_merge_type:
                            ...
                            if generation_type == "downsampled":
                                ...
                                image_feature = torch.cat((downsampled_base_image_feature2, downsampled_base_image_feature, base_image_feature), dim=0)
                            elif generation_type == "recursion_stage1":
                                if type(recursed_features) is list:
                                    recursed_features = torch.cat(recursed_features)
                                image_feature = torch.cat((downsampled_base_image_feature2, downsampled_base_image_feature, recursed_features), dim=0)
                            elif generation_type == "recursion_stage2":
                                if type(recursed_features) is list:
                                    recursed_features = torch.cat(recursed_features)
                                else:
                                    print("no recursed features")
                                image_feature = torch.cat((downsampled_base_image_feature2, downsampled_base_image_feature, base_image_feature, recursed_features), dim=0)
                            elif generation_type == "downsampled_tot":
                                image_feature = torch.cat((downsampled_base_image_feature2, downsampled_base_image_feature, base_image_feature, image_feature), dim=0)
