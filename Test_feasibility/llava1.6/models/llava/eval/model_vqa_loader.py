...
def eval_model(args):
    ...
    for (input_ids, image, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        if args.recursion_type == "all":
            print("recursion")
            heat_torch_stack, img_with_attn, ret_attn = get_heatmap(model, output_ids, tokenizer, cur_prompt, image, input_ids)
            print(len(ret_attn))
            del output_ids
...
if __name__ == "__main__":
    ...
    parser.add_argument("--recursion_threshold", type=float, default=0.1)
