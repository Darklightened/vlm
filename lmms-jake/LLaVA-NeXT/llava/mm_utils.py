...
def get_sinusoidal_embeddings(num_positions, embed_dim, device):
    """
    Generate sinusoidal positional embeddings.
    """
    position = torch.arange(0, num_positions, dtype=torch.float32, device=device).unsqueeze(1)
    div_term = torch.exp(torch.arange(0, embed_dim, 2, dtype=torch.float32, device=device) * 
                         -(math.log(10000.0) / embed_dim))
    sinusoidal_embeddings = torch.zeros((num_positions, embed_dim), dtype=torch.float32, device=device)
...
def init_downsampled_vision_towers(vision_tower, stages, positional_embedding_type, device):
    ...
    for stage in stages:
        ...
        elif positional_embedding_type == "reduced_sin":
            print("Reduced embedding type with sinusoidal embeddings.")

            # Load pretrained embedding
            original_embedding = downsampled_vision_towers[stage].vision_tower.vision_model.embeddings.position_embedding.weight.data
            original_embedding = original_embedding[:num_positions]  # Truncate to num_positions if needed

            # Generate sinusoidal embeddings
            sinusoidal_embedding = get_sinusoidal_embeddings(num_positions, embed_dim, device)
            # Ensure both tensors are on the same device before combining
            original_embedding = original_embedding.to(device)
            sinusoidal_embedding = sinusoidal_embedding.to(device)

            # Combine pretrained embedding with sinusoidal embedding
            combined_embedding = (original_embedding.to(dtype=torch.float32) + sinusoidal_embedding) / 2.0  # Averaging

            # Replace positional embedding with the new combined embedding
            downsampled_vision_towers[stage].vision_tower.vision_model.embeddings.position_embedding = torch.nn.Embedding(
                num_positions, embed_dim).to(dtype=torch.float16, device=device)
            downsampled_vision_towers[stage].vision_tower.vision_model.embeddings.position_embedding.weight.data.copy_(
                combined_embedding.to(dtype=torch.float16)
            )
            downsampled_vision_towers[stage].vision_tower.vision_model.embeddings = \
                downsampled_vision_towers[stage].vision_tower.vision_model.embeddings.to(device)
