...
def divide_to_patches(image, patch_size):
    ...
    for h, i in enumerate(range(0, height, patch_size)):
        for w, j in enumerate(range(0, width, patch_size)):
...
def process_anyres_image(image, processor, grid_pinpoints):
    ...
    patches, h, w = divide_to_patches(image_padded, processor.crop_size["height"])
...
def process_images(images, image_processor, model_cfg):
    ...
    new_images = []
    assert len(images) == 1, "input only single image"
...
def get_heatmap(
    ...
    for stage in stages:
        ...
        patches_in_stage = image_mask[stage].sum().item()
    ...
    # Initialize results
    ret_attn_list = [torch.zeros_like(image_mask[current_stage]) for _ in output_token_inds]
    stage_attn_list = []
    ...
    for stage in stages:
        ...
        grid_size = image_mask[stage].shape[0]
        num_patches = int(grid_size ** 2)
        # if stage == 1:
        #     grid_size *= stage1_grid[0]
            
        used_token_list = image_mask[stage]

        if stage == 1:
            blocks = []
            step = int(grid_size / stage1_grid)
            for h in range(0, grid_size, step):
                for w in range(0, grid_size, step):
                    block = used_token_list[h : h + step,  w : w + step].flatten()
                    blocks.append(block)

            used_token_list = torch.cat(blocks, dim=0).tolist()
        ...
        if stage == 1:
            ...
            for stage1_idx in range(stage1_grid * stage1_grid):
                    ...
                    if sum(used_list) != 0:
                        ...
                        for idx in range(len(used_list)):
                            ...
                            if used_list[idx] == 1:
                                ...
                                # vis_attn = vis_attn.reshape(grid_size//stage1_grid[0], grid_size//stage1_grid[0])
                        ...
                        # attn_over_image = torch.nn.functional.interpolate(attn_over_image.unsqueeze(0).unsqueeze(0), size=(h//stage1_grid[0], w//stage1_grid[0]), mode='nearest').squeeze()
                        stage1_dict[stage1_idx].append(attn_over_image)
                    else:
                        # attn_over_image = torch.zeros_like(ret_attn_list[i])
                        attn_over_image = torch.zeros_like(ret_attn_list[i])
                        attn_over_image = torch.nn.functional.interpolate(attn_over_image.unsqueeze(0).unsqueeze(0), size=(grid_size//stage1_grid[0], grid_size//stage1_grid[0]), mode='nearest').squeeze()
                        stage1_dict[stage1_idx].append(attn_over_image)
                vision_token_start_1 += int(sum(used_list))
            vision_token_end = vision_token_end_1

            for idx in range(len(output_token_inds)):
                rows = []
                for h in range(stage1_grid):
                    row = torch.hstack([stage1_dict[f][idx] for f in range(h*stage1_grid, h*stage1_grid + stage1_grid)])
                    rows.append(row)
                attn_over_image = torch.vstack(rows)
            ...
            for i in range(len(output_token_inds)):
                ...
                if len(used_token_list) != 0:
                    ...
                    attn_over_image = attn_over_image.to(device=model.device)
                    attn_over_image = attn_over_image * image_mask[stage]
                    attn_over_image = torch.nn.functional.interpolate(attn_over_image.unsqueeze(0).unsqueeze(0), size=(h, w), mode='bilinear', align_corners=True).squeeze()
                else:
                    attn_over_image = torch.zeros_like(ret_attn_list[i])
    ...
    if image is not None and save_path is not None:
        ...
        for i, (raw_attn, vis_attn) in enumerate(zip(ret_attn_list, vis_attn_list)):
            ...
            plt.axis("off")
            plt.savefig(f"{save_path}/{str(i).zfill(4)}_stage{current_stage}_{token_string}.png")
            plt.close('all')
