...
df['Cumulative Confidences'] = df['Cumulative Confidences'].apply(lambda confs: extract_numbers_from_tensors(confs))
...
df['Cumulative Confidences'] = df['Cumulative Confidences'].apply(
    lambda x: extract_numbers_from_tensors(ast.literal_eval(x)) if isinstance(x, str) else x
)
...
df.to_csv('processed_output_separate_stages.csv', index=False)

print("Processed data saved to 'processed_output_separate_stages.csv'")
