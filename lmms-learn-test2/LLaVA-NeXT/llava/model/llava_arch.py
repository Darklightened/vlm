        ...
        if type(images) is list or images.ndim == 5:
            ...
            if mm_patch_merge_type == "flat":
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                    ...
                    elif image_feature.shape[0] > 1 and generation_type is not None:
                        '''
                        AIDAS VLM
                        (Migrating VLM Hallucination)
                        
                        element-wise product each stage-n-features by n-image-mask.
                        use only non-zero features.
                        '''
                        ...
                        if image_aspect_ratio == "anyres" or "anyres_max" in image_aspect_ratio:
                            ...
                            stage1_feature = stage1_feature.view(num_patch_height, num_patch_width, height, width, -1)
                        else:
                            stage1_feature = stage1_feature.view(2, 2, height, width, -1)

                        if "maxpool2x2" in mm_patch_merge_type:
                            stage1_feature = stage1_feature.permute(4, 0, 2, 1, 3).contiguous()
                            stage1_feature = stage1_feature.flatten(1, 2).flatten(2, 3)
                            stage1_feature = nn.functional.max_pool2d(stage1_feature, 2)
                            stage1_feature = stage1_feature.flatten(1, 2).transpose(0, 1)
                        elif "unpad" in mm_patch_merge_type and "anyres_max" in image_aspect_ratio and matched_anyres_max_num_patches:
                            unit = stage1_feature.shape[2]
                            stage1_feature = stage1_feature.permute(4, 0, 2, 1, 3).contiguous()
                            stage1_feature = stage1_feature.flatten(1, 2).flatten(2, 3)
                            stage1_feature = unpad_image(stage1_feature, image_sizes[image_idx])
                            c, h, w = stage1_feature.shape
                            times = math.sqrt(h * w / (max_num_patches * unit**2))
                            if times > 1.1:
                                stage1_feature = stage1_feature[None]
                                stage1_feature = nn.functional.interpolate(stage1_feature, [int(h // times), int(w // times)], mode="bilinear")[0]
                            stage1_feature = torch.cat((stage1_feature, self.model.image_newline[:, None, None].expand(*stage1_feature.shape[:-1], 1).to(stage1_feature.device)), dim=-1)
                            stage1_feature = stage1_feature.flatten(1, 2).transpose(0, 1)
                        ...
                        # if "recursion" in generation_type:
                        if True:
                            ...
                            if isinstance(image_mask, dict):
                                for downsampled_feature, stage in zip(encoded_downsampled_image_features, stages):
                                    ...
                                    print(f"mask grad fn: {mask.grad_fn}")
                                    print(f"Is mask detached? {mask.is_leaf}")
                                    print(f"mask requires_grad: {mask.requires_grad}")
                                    ...
                                    print(f"resized_mask grad fn: {resized_mask.grad_fn}")
                                    ...
                                    downsampled_feature = downsampled_feature[~torch.all(downsampled_feature < 0.1, dim=1)]
                                ...
                                mask0 = image_mask[str(0)]
                                print(f"Is mask0 detached? {mask0.is_leaf}")
                                print(f"mask0 grad fn: {mask0.grad_fn}")
                                print(f"mask0 requires grad {mask0.requires_grad}")
                                resized_mask0 = torch.nn.functional.interpolate(mask0.unsqueeze(0).unsqueeze(0), size=(patches_per_side, patches_per_side), mode='nearest')
                                resized_mask0 = resized_mask0.squeeze().unsqueeze(-1)
                                print(f"resized_mask0 grad fn: {resized_mask0.grad_fn}")
                                stage0_feature = stage0_feature * resized_mask0
                                stage0_feature = stage0_feature.view(patches_per_side ** 2, -1)
                                stage0_feature = stage0_feature[~torch.all(stage0_feature < 0.1, dim=1)]
                                print(f"stage0_feature: {stage0_feature}")
                                print(f"stage0_feature grad fn: {stage0_feature.grad_fn}")
                                ...
                                mask1 = image_mask[str(1)]
                                print(f"Is mask1 detached? {mask1.is_leaf}")
                                print(f"mask1 grad fn: {mask1.grad_fn}")
                                resized_mask1 = torch.nn.functional.interpolate(mask1.unsqueeze(0).unsqueeze(0), size=(patches_per_side, patches_per_side), mode='nearest')
                                print(f"resized_mask1 grad fn: {resized_mask1.grad_fn}")
                                resized_mask1 = resized_mask1.squeeze().unsqueeze(-1)
                                print(f"resized_mask1 grad fn: {resized_mask1.grad_fn}")
                                stage1_feature = stage1_feature * resized_mask1
                                print(f"stage1_feature grad fn: {stage1_feature.grad_fn}")
                                stage1_feature = stage1_feature.view(patches_per_side ** 2, -1)
                                print(f"stage1_feature grad fn: {stage1_feature.grad_fn}")
                                stage1_feature = stage1_feature[~torch.all(stage1_feature < 0.1, dim=1)]
                                print(f"stage1_feature grad fn: {stage1_feature.grad_fn}")
