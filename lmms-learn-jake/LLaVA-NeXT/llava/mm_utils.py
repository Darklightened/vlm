...
def norm(ret_attn):
    ...
    min = ret_attn.min()
    temp = (ret_attn-min) / (max-min + 1e-08)  
    ret_attn = temp
    return ret_attn

def norm_softmax(ret_attn, temperature=1.0):
    scaled_attn = ret_attn / temperature    
    ret_attn = torch.softmax(scaled_attn.flatten(), dim=0).view(ret_attn.size())
