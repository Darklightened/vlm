...
def get_heatmap(
    ...
    padded_image_mask = combine_image_and_pad_mask(image_mask, pad_mask)
    ...
    for stage in stages:
        ...
        mask = torch.nn.functional.interpolate(padded_image_mask[stage].unsqueeze(0).unsqueeze(0), size=(mask_grid, mask_grid), mode='nearest')
    ...
    # Initialize results
    ret_attn_list = [torch.zeros_like(padded_image_mask[0]) for _ in output_token_inds]
    stage_attn_list = []
    ...
    for stage in stages:
        ...
        used_token_list = torch.nn.functional.interpolate(padded_image_mask[stage].unsqueeze(0).unsqueeze(0), size=(grid_size, grid_size), mode='nearest').squeeze()
        ...
        if stage == 1:
            ...
            for idx, features in enumerate(zip(stage1_dict[0], stage1_dict[1], stage1_dict[2], stage1_dict[3])):
                ...
                attn_over_image = attn_over_image * padded_image_mask[stage]
    ...
    if image is not None and save_path is not None:
        ...
        tensor_norm = ret_attn.clone()
        tensor_norm = tensor_norm - tensor_norm.min()
        tensor_norm = tensor_norm / tensor_norm.max()
        tensor_norm[tensor_norm>0.8] = 0
        tensor_norm = tensor_norm / tensor_norm.max()
        vis_attn_list.append(tensor_norm)
        
        ret_attn_list.append(stage_attn)
        # tensor_norm = descending_indices_2d(stage_attn)
        tensor_norm = stage_attn.clone()
        tensor_norm = tensor_norm - tensor_norm.min()
        tensor_norm = tensor_norm / tensor_norm.max()
        vis_attn_list.append(tensor_norm)
...
def combine_image_and_pad_mask(image_masks, pad_masks):
    new_image_masks = dict()
    for stage in image_masks.keys():
        new_image_masks[stage] = image_masks[stage] * pad_masks[stage]
    return new_image_masks