...
# Load the CSV file
file_path = "/workspace/vlm/lmms-eval-logit/wandb_grid5.csv"  # Replace with the path to your CSV file
output_dir = "/workspace/vlm/lmms-eval-logit/figs/bilinear/grid5"  # Directory to save the plots
os.makedirs(output_dir, exist_ok=True)  # Create directory if it doesn't exist
...
default_values = {
    "pope_pop/pope_f1_score": 0.8647,
    "vqav2_val_lite/exact_match": 0.7636,
    "mmbench_en_dev_lite/gpt_eval_score": 75.76,
    "mmstar/average": 0.3725,
}
...
mmstar_task = "mmstar/average"  # Use only the "average" column for MMStar

# Filter datasets based on available columns
vqa_data = df.dropna(subset=["vqav2_val_lite/exact_match"]) if "vqav2_val_lite/exact_match" in df.columns else None
other_tasks_data = df.dropna(subset=["pope_pop/pope_f1_score", "mmbench_en_dev_lite/gpt_eval_score", "mmstar/average"])

# Function to plot individual tasks
def plot_task(data, task_col, task_name, default_value, output_dir):
    if task_col not in data.columns:
        print(f"Column {task_col} not found in the dataset. Skipping...")
        return

    print(f"\nAnalyzing trends for {task_name}...")
    for stage in ['stage_minus1', 'stage_0', 'stage_1']:
        stage_means = data.groupby(stage)[task_col].mean()
        if stage_means.empty:
            print(f"No data for {task_name} at {stage}. Skipping...")
            continue

        plt.figure()
        plt.plot(stage_means.index, stage_means.values, marker='o', label="Performance")
        plt.axhline(y=default_value, color='red', linestyle='--', label="Default Value")
        plt.title(f"{task_name} vs {stage}")
        plt.xlabel(f"{stage} Threshold")
        plt.ylabel(task_name)
        plt.legend()
...
if vqa_data is not None:
    plot_task(vqa_data, "vqav2_val_lite/exact_match", "VQA Exact Match", default_values["vqav2_val_lite/exact_match"], output_dir)

# Plot other tasks
for task_col, task_name in tasks.items():
    if task_col == "vqav2_val_lite/exact_match":
        continue  # Already handled above
    plot_task(other_tasks_data, task_col, task_name, default_values[task_col], output_dir)

# Plot MMStar average
if mmstar_task in other_tasks_data.columns:
    plot_task(other_tasks_data, mmstar_task, "MMStar Average", default_values[mmstar_task], output_dir)

print("Plots with default values saved successfully.")