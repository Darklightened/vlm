    ...
    def generate_recursive(
        ...
        **kwargs,
        ...
        for token_idx in range(max_length):   
            ...
            temp_token = torch.argmax(final_logit, dim=-1)
            temp_text = tokenizer.batch_decode([temp_token], skip_special_tokens=True)[0]
            ...
            if self.use_noised_for_contrastive:
                ...
                temp_token = torch.argmax(noised_scores, dim=-1)
                temp_text = tokenizer.batch_decode([temp_token], skip_special_tokens=True)[0]

                # final_logit += self.contrastive_alphas[-1]*(final_logit - noised_scores)

                cutoff = torch.log(torch.tensor(0.2)) + final_logit.max(dim=-1, keepdim=True).values
        
                diffs = (1+1.0)*final_logit - 1.0*noised_scores
                cd_logits = diffs.masked_fill(final_logit < cutoff, -float("inf"))

                ####
                # cutoff = torch.tensor(0.5, device=final_logit.device) * final_logit.max()
                ...
                best_token = torch.argmax(cd_logits, dim=-1)
