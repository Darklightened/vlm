...
def normalize_attention_list(ret_attn_list):
    normalized_list = []
    
    for tensor in ret_attn_list:
        min_val = tensor.min()
        max_val = tensor.max()
        
        # Avoid division by zero if max_val equals min_val
        if max_val == min_val:
            normalized_tensor = torch.zeros_like(tensor)  # or ones if you want all 1s
        else:
...
def run():
    ...
    for img_idx, i in enumerate(range(len(dataset))):
        ...
        for cat_idx, cat_name in enumerate(predict_list):
            ...
            ##################### 2nd Stage ##########################
            print(f'2nd stage-> ret_atten[1].shape: {ret_attn[1].shape}')
            ...
            print(f'3rd stage-> ret_atten[1].shape: {ret_attn[1].shape}')
            med = torch.stack(ret_attn, dim=0)
            med = med.mean(dim=0)
            attn = ret_attn[1] - med
            attn = torch.relu(attn)
            ...
            is_positive = cat_idx < k
            hallucination = (is_positive and answer == "no") or (not is_positive and answer == "yes")
            with open(f"/home/aidas_intern_1/woohyeon/llava1.6/jake_result/jake_3stage.csv", "a") as f:
