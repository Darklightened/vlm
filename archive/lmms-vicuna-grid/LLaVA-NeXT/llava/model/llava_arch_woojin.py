...
class LlavaMetaModel:
    ...
    def get_downsampled_vision_towers(self):
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_downsampled_vision_towers(self):
        return self.get_model().get_downsampled_vision_towers()
    ...
    def encode_downsampled_images(self, vision_tower_idx, images):
        image_features = self.get_model().get_downsampled_vision_tower()[vision_tower_idx](images)
        # image_features = self.get_model().vision_resampler(image_features, images=images)
        image_features = self.get_model().mm_projector(image_features)
        return image_features
    ...
    current_stage=None, all_image_sizes=None, recursed_image_sizes=None):        
        ...
        if type(images) is list or images.ndim == 5:
            ...
            if downsampled_images is not None:
                assert generation_type not in ["default", None]
                encoded_downsampled_image_features = [self.encode_downsampled_images(idx, downsampled_image.squeeze(0)) for idx, downsampled_image in enumerate(downsampled_images)]
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        # use all image sizes in all_image_sizes (no recursion)
                            ...
                            if 336 in all_image_sizes:
                                base_image_features = torch.cat((base_image_features, base_image_feature), dim=0)
                            
                            if 672 in all_image_sizes:
                                image_feature = image_feature.permute(0, 2, 1, 3, 4).contiguous()
                                image_feature = image_feature.flatten(0, 3)
                                base_image_features = torch.cat((base_image_features, image_feature), dim=0)                            
