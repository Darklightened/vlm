...
def rank_based_normalization_2d(data):
    """
    Rank-based normalization for a 2D PyTorch tensor.
    Maps data to a standard normal distribution using ranks and inverse CDF,
    while preserving the original 2D shape.
    ...
    """
    # 1. Flatten the data to 1D
    original_shape = data.shape
    flattened_data = data.flatten()

    # 2. Perform rank-based normalization
    sorted_indices = torch.argsort(flattened_data)  # Get sorted indices
    ranks = torch.zeros_like(flattened_data)
    ranks[sorted_indices] = torch.arange(1, len(flattened_data) + 1, dtype=torch.float32, device=data.device)

    # Normalize ranks to (0, 1]
    ranks /= len(flattened_data) + 1

    # Transform ranks to standard normal distribution
    normal_quantiles = torch.distributions.Normal(0, 1).icdf(ranks)

    # 3. Reshape back to the original 2D shape
    normalized_data = normal_quantiles.view(original_shape)
...
def get_heatmap(
        ...
        if stage == 1:
            ...
            for stage1_idx in range(4):
                ...
                for i in range(len(output_token_inds)):
                    ...
                    start = vision_token_start + 576 * stage1_idx
                    end = start + 576
                    attn_weights_over_vis_tokens = llm_attn_matrix[target_token_ind][start:end]        
                    attn_weights_over_vis_tokens = attn_weights_over_vis_tokens / attn_weights_over_vis_tokens.sum()
                    
                    used_list = used_token_list[576 * stage1_idx : 576 * (stage1_idx + 1)]
            ...
            for idx, features in enumerate(zip(stage1_dict[0], stage1_dict[1], stage1_dict[2], stage1_dict[3])):
                ...
                ret = torch.vstack([top, bot])
                ret_attn_list[idx] = ret_attn_list[idx] + ret
    ...
    if image is not None and save_path is not None:
        ...
        for i, token_attn in enumerate(ret_attn_list):
            if i == len(ret_attn_list) - 1:
                token_string = "total"
            else:
