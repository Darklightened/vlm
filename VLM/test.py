...
def get_sinusoidal_positional_encoding(num_positions, embed_dim, device):
    """Generates sinusoidal positional encoding matrix."""
    position_ids = torch.arange(num_positions, dtype=torch.float, device=device).unsqueeze(1)
    div_term = torch.exp(torch.arange(0, embed_dim, 2, dtype=torch.float, device=device) * (-math.log(10000.0) / embed_dim))

    sinusoidal_embedding = torch.zeros(num_positions, embed_dim, device=device)
    sinusoidal_embedding[:, 0::2] = torch.sin(position_ids * div_term)
    sinusoidal_embedding[:, 1::2] = torch.cos(position_ids * div_term)

    return sinusoidal_embedding

def calculate_hallucination_accuracy(file_path):
    df = pd.read_csv(file_path)

    hallucination_true_count = df['hallucination'].sum()

    total_count = 14631

    accuracy = hallucination_true_count / total_count
...
def main():
    ...
    parser.add_argument("--output_path", default='/home/aidas_intern_1/woohyeon/TEST/test.csv' , type=str)
    args = parser.parse_args()
    args.output_path = f'/home/aidas_intern_1/woohyeon/TEST/test_{args.image_size}.csv'
    ...
    # image_processor.crop_size = {'height' : args.image_size, 'width' : args.image_size}
    ...
    sinusoidal_position_embedding = get_sinusoidal_positional_encoding(num_positions, embed_dim, device)
    ...
    model.model.vision_tower.vision_tower.vision_model.embeddings.position_embedding.weight.data = sinusoidal_position_embedding
    ...
    calculate_hallucination_accuracy(args.output_path)
